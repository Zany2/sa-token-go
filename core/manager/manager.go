...
type TokenInfo struct {
	...
	ActiveTime int64  `json:"activeTime"`    // Last active time | 最后活跃时间戳（续期时间）
...
type DisableInfo struct {
	...
	DisableReason string `json:"disableReason"` // Reason for account disable | 封禁原因
}
...
func NewManager(
	...
	// Use default logger if logger is nil | 如果未传入日志记录器，则使用默认日志记录器
	if logger == nil {
...
func (m *Manager) IsLogin(ctx context.Context, tokenValue string) bool {
	...
	if m.config.ActiveTimeout > 0 {
		...
		if now-info.ActiveTime > m.config.ActiveTimeout {
			if sess, err := m.GetSessionByToken(ctx, info.LoginID); err == nil && sess != nil {
				_ = sess.RemoveTerminalByToken(ctx, tokenValue, m.getExpiration())
			}
			_ = m.removeTokenChain(ctx, tokenValue, listener.EventKickout)
			return false
		}
	}
	...
	if m.config.AutoRenew && m.config.Timeout > 0 {
		...
		if ttl, err := m.storage.TTL(ctx, tokenKey); err == nil {
			...
			if ttlSeconds > 0 && (m.config.MaxRefresh <= 0 || ttlSeconds <= m.config.MaxRefresh) && (m.config.RenewInterval <= 0 || !m.storage.Exists(ctx, m.getRenewKey(tokenValue))) {
				// 构造续期函数
				renewFunc := func() {
					m.renewToken(context.Background(), tokenValue, nil, nil)
				}
...
func (m *Manager) Disable(ctx context.Context, loginID string, duration time.Duration, reason ...string) error {
	...
	// Delete tokens from storage | 删除Token
	err = m.storage.Delete(ctx, tokenList...)
	if err != nil {
		return err
	}
...
// GetDisableTTL Gets remaining disable time in | 获取账号剩余封禁时间
...
func (m *Manager) GetTokenValueListByLoginID(ctx context.Context, loginID string, checkAlive ...bool) ([]string, error) {
	...
	tokenList := make([]string, len(sess.TerminalInfos))
	for _, info := range sess.TerminalInfos {
		if len(checkAlive) > 0 && checkAlive[0] {
...
func (m *Manager) GetTokenValueListByDeviceAndDeviceId(ctx context.Context, loginID string, device string, checkAlive ...bool) ([]string, error) {
	sess, err := m.GetSession(ctx, loginID)
	if err != nil {
		return nil, err
	}
...
func (m *Manager) SetTokenTag(ctx context.Context, tokenValue, tag string) error {
	// Get token info | 获取Token信息
	tokenInfo, err := m.getTokenInfo(ctx, tokenValue)
	if err != nil {
		return err
	...
	// Encode token info | 编码Token信息
	encodeData, err := m.serializer.Encode(tokenInfo)
	if err != nil {
		return fmt.Errorf("%w: %v", core.ErrSerializeFailed, err)
	}

	// Save token info | 保存Token信息
	err = m.storage.Set(ctx, m.getTokenKey(tokenValue), encodeData, m.getExpiration())
	if err != nil {
		return fmt.Errorf("%w: %v", core.ErrStorageUnavailable, err)
	}
...
func (m *Manager) GetTokenTag(ctx context.Context, tokenValue string) (string, error) {
	// Get token info | 获取Token信息
	tokenInfo, err := m.getTokenInfo(ctx, tokenValue)
	if err != nil {
		return "", err
...
func (m *Manager) getTokenInfo(ctx context.Context, tokenValue string, checkState ...bool) (*TokenInfo, error) {
	...
	if err != nil {
		m.logger.Errorf("Manager getTokenInfo err: %v", err)
		return nil, fmt.Errorf("%w: %v", core.ErrStorageUnavailable, err)
	}
	// If data is nil, the token is expired | 如果数据为nil，则Token已过期
	if data == nil {
...
func (m *Manager) renewToken(ctx context.Context, tokenValue string, info *TokenInfo, sess *session.Session) {
	...
	if m.config.RenewInterval > 0 {
		...
		if err != nil {
			m.logger.Errorf("Manager renewToken Storage RenewInterval err: %v", err)
			return
		}
	}
}
